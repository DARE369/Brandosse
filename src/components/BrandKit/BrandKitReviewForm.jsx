...
const TAB_CONFIG = [
  {
    key: 'Basics',
    fields: ['brand_name', 'industry', 'tagline', 'website_url', 'target_audience', 'audience_age_range', 'audience_locations', 'primary_language'],
  ...
  {
    key: 'Visual Style',
    fields: ['visual_style_keywords', 'color_palette', 'typography_notes', 'photo_style_notes', 'avoid_visual_elements'],
    ...
    return value
      .map((entry) => {
        if (typeof entry === 'string') {
          ...
          return {
            hex: isHexColor(token) ? token : '#6366f1',
            name: isHexColor(token) ? '' : token,
            usage: '',
          };
        }
    ...
    <div className="tag-input-wrapper">
      {tags.map((tag) => (
        <span key={tag} className="tag-chip">
          {tag}
          <button type="button" onClick={() => onChange(tags.filter((item) => item !== tag))}>x</button>
        </span>
      ))}
      <input
        className="bk-input"
      ...
      />
      <button className="bk-btn-ghost bk-btn-sm" type="button" onClick={addTag}>Add</button>
    </div>
  );
}
  ...
  return (
    <div className="bk-palette">
      {colors.map((color, index) => (
        <div key={`${color.hex || ''}-${index}`} className="bk-palette-row">
          <input
            type="color"
            className="bk-color-swatch"
          ...
          <button
            ...
            type="button"
            onClick={() => onChange(colors.filter((_, idx) => idx !== index))}
          >
            x
          </button>
        </div>
      ...
      {colors.length < 8 && (
        <button className="bk-btn-ghost" type="button" onClick={addColor}>
          Add color
        </button>
      )}
    </div>
...
function FieldShell({ label, children, confidence, isTier1 = false }) {
  const flagged = confidence === 'low' || confidence === 'inferred';

  return (
    <div className={`bk-field ${flagged ? 'bk-field-flagged' : ''}`}>
      <label className="bk-label">
        {label}
        {isTier1 && <span className="bk-required-dot" title="Important for AI quality" />}
      </label>
      {children}
      {flagged && (
        <p className="bk-confidence-note">
...
export default function BrandKitReviewForm({
  ...
  const renderBasics = () => (
    <div className="bk-review-section">
      <FieldShell label="Brand Name" confidence={confidenceMap.brand_name} isTier1>
        <input className="bk-input" type="text" value={form.brand_name} onChange={(event) => setField('brand_name', event.target.value)} />
      </FieldShell>
      <FieldShell label="Industry" confidence={confidenceMap.industry}>
        <select className="bk-select" value={form.industry} onChange={(event) => setField('industry', event.target.value)}>
          <option value="">Select</option>
          {INDUSTRIES.map((industry) => <option key={industry} value={industry}>{industry}</option>)}
        </select>
      </FieldShell>
      <FieldShell label="Tagline" confidence={confidenceMap.tagline}>
        <input className="bk-input" type="text" value={form.tagline} onChange={(event) => setField('tagline', event.target.value)} />
      </FieldShell>
      <FieldShell label="Website URL" confidence={confidenceMap.website_url}>
        <input className="bk-input" type="url" value={form.website_url} onChange={(event) => setField('website_url', event.target.value)} />
      </FieldShell>
      <FieldShell label="Target Audience" confidence={confidenceMap.target_audience} isTier1>
        <textarea className="bk-textarea" rows={3} value={form.target_audience} onChange={(event) => setField('target_audience', event.target.value)} />
      </FieldShell>
      <FieldShell label="Audience Age Range" confidence={confidenceMap.audience_age_range}>
        <input className="bk-input" type="text" value={form.audience_age_range} onChange={(event) => setField('audience_age_range', event.target.value)} />
      </FieldShell>
      ...
      <FieldShell label="Primary Language" confidence={confidenceMap.primary_language}>
        <select className="bk-select" value={form.primary_language} onChange={(event) => setField('primary_language', event.target.value)}>
  ...
  const renderVoice = () => (
    <div className="bk-review-section">
      <FieldShell label="Brand Voice" confidence={confidenceMap.brand_voice} isTier1>
        <div className="bk-pills">
          {VOICES.map((voice) => (
            <button
              key={voice}
              className={`bk-pill ${form.brand_voice === voice ? 'active' : ''}`}
              type="button"
              onClick={() => setField('brand_voice', voice)}
            >
              {voice}
            </button>
          ))}
      ...
      <FieldShell label="Writing Style Notes" confidence={confidenceMap.writing_style_notes}>
        <textarea className="bk-textarea" rows={3} value={form.writing_style_notes} onChange={(event) => setField('writing_style_notes', event.target.value)} />
      </FieldShell>
      ...
      <FieldShell label="Emoji Usage" confidence={confidenceMap.emoji_usage}>
        <div className="bk-pills">
          {EMOJIS.map((emojiUsage) => (
            <button
              key={emojiUsage}
              className={`bk-pill ${form.emoji_usage === emojiUsage ? 'active' : ''}`}
              type="button"
              onClick={() => setField('emoji_usage', emojiUsage)}
            >
              {emojiUsage}
            </button>
          ))}
      ...
      <FieldShell label="Call to Action Style" confidence={confidenceMap.call_to_action_style}>
        <select className="bk-select" value={form.call_to_action_style} onChange={(event) => setField('call_to_action_style', event.target.value)}>
          <option value="">Select</option>
  ...
  const renderGuardrails = () => (
    <div className="bk-review-section">
      <FieldShell label="Content Restrictions" confidence={confidenceMap.content_restrictions} isTier1>
      ...
      <FieldShell label="Legal Disclaimers" confidence={confidenceMap.legal_disclaimers}>
        <textarea className="bk-textarea" rows={3} value={form.legal_disclaimers} onChange={(event) => setField('legal_disclaimers', event.target.value)} />
      </FieldShell>
      <FieldShell label="Brand Safe Only" confidence={confidenceMap.brand_safe_only}>
        <label className="bk-toggle-label">
      ...
      <div className="bk-range-row">
        <FieldShell label="Min Caption Words" confidence={confidenceMap.min_caption_words}>
          <input className="bk-input bk-input-sm" type="number" min={0} max={500} value={form.min_caption_words} onChange={(event) => setField('min_caption_words', Number(event.target.value))} />
        </FieldShell>
        <FieldShell label="Max Caption Words" confidence={confidenceMap.max_caption_words}>
          <input className="bk-input bk-input-sm" type="number" min={0} max={1000} value={form.max_caption_words} onChange={(event) => setField('max_caption_words', Number(event.target.value))} />
        </FieldShell>
        <FieldShell label="Max Hashtags" confidence={confidenceMap.max_hashtags}>
          <input className="bk-input bk-input-sm" type="number" min={0} max={30} value={form.max_hashtags} onChange={(event) => setField('max_hashtags', Number(event.target.value))} />
        </FieldShell>
      </div>
    </div>
  );
  ...
  const renderVisual = () => (
    <div className="bk-review-section">
      ...
      <FieldShell label="Typography Notes" confidence={confidenceMap.typography_notes}>
        <textarea className="bk-textarea" rows={2} value={form.typography_notes} onChange={(event) => setField('typography_notes', event.target.value)} />
      </FieldShell>
      <FieldShell label="Photo Style Notes" confidence={confidenceMap.photo_style_notes}>
        <textarea className="bk-textarea" rows={2} value={form.photo_style_notes} onChange={(event) => setField('photo_style_notes', event.target.value)} />
      </FieldShell>
      <FieldShell label="Avoid Visual Elements" confidence={confidenceMap.avoid_visual_elements}>
  ...
  const renderAssets = () => (
    <div className="bk-review-section">
      {userId && currentBrandKitId ? (
        ...
        <p className="bk-hint">Save your Brand Kit once to enable asset uploads.</p>
      )}
    </div>
  );
  ...
  return (
    <>
      <div className="bk-review-layout">
        <aside className="bk-review-sidebar">
          <h3 className="bk-review-sidebar-title">Brand Kit Sections</h3>
          {TAB_CONFIG.map((tab) => {
            ...
            const meta = tab.key === 'Assets'
              ...
              : hasWarning
                ? 'Missing critical'
                : `${metrics?.filled || 0}/${metrics?.total || 0} fields`;

            return (
              <button
                key={tab.key}
                className={`bk-review-tab ${activeTab === tab.key ? 'active' : ''} ${hasWarning ? 'has-warning' : ''}`}
                onClick={() => setActiveTab(tab.key)}
                type="button"
              >
                <span className="bk-review-tab-label">{tab.key}</span>
                <span className="bk-review-tab-meta">{meta}</span>
              </button>
            );
          })}
        </aside>

        <section className="bk-review-panel">
          <div className="bk-review-panel-header">
            <div>
              <h2 className="bk-review-panel-title">{headerText}</h2>
              <p className="bk-review-panel-desc">
                {TAB_CONFIG.find((tab) => tab.key === activeTab)?.description}
              </p>
            </div>
            <button className="bk-btn-primary" type="button" onClick={handleSubmit} disabled={isSaving}>
              {isSaving ? 'Saving...' : (brandKit?.setup_completed ? 'Save changes' : 'Confirm and activate')}
            </button>
          </div>

          {saveError && <div className="bk-error-banner" role="alert">{saveError}</div>}
