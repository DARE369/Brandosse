...
| **D5** | Stale comment references a deleted route | `app/api/auth/zernio/connect/route.js` header says it mirrors `app/api/auth/oauth/route.js`; that file does not exist. | Misleads the next reader into thinking a direct path exists. Delete with the rest. |
