...
| App shell and context | One shared app chrome (header, nav, mobile drawer, credits, notifications, avatar, skip link), theme, logout provider, workspace sync, global mock publish modal, protected app routing. The legacy `UserNavbar`/`UserSidebar` pair was deleted 2026-08-24 once every personal route moved to `AppShell`. | `src/ui-v2/shell/AppShell.jsx`, `src/ui-v2/shell/navItems.js`, `src/Context/LogoutContext.jsx`, `src/Context/ThemeContext.jsx` |
