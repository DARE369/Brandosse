...
| Runtime stability fixes | Key DB/runtime issues were resolved (brand_kit race, navbar post query mismatch, status write normalization in core admin paths). | `docs/database-consistency-audit.md`, `src/stores/BrandKitStore.js`, `src/ui-v2/shell/AppShell.jsx` (replaced the deleted `UserNavbar.jsx`), admin moderation files listed in audit | Reduced demo risk and fewer hard runtime breaks during normal use. |
